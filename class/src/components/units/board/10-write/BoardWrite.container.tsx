...
export default function BoardWrite(props: IBoardWriteProps) {
  ...
  const onClickSubmit = async () => {
    ...
    router.push(`/10-01-typescript-boards/${result.data.createBoard.number}`); //등록되면 상세페이지로 이동
  };
  ...
  const onClickUpdate = async () => {
    ...
    router.push(`/10-01-typescript-boards/${result.data.updateBoard.number}`); //수정되면 수정한 상세페이지로 이동
  };
